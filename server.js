 ...
 * Formato: /m3u/:slug.m3u (slug = nombre de usuario personalizado)
 * Mantiene compatibilidad hacia atrás con UUID
 */
app.get('/m3u/:slug.m3u', async (req, res) => {
  try {
    ...
    // Obtener información del usuario por UUID o slug
    let user;
    if (isUUID) {
      user = await db.getUserById(slug);
    } else {
      user = await db.getUserBySlug(slug);
    }
    ...
    // Registrar acceso en la base de datos (incrementa contador acumulativo)
    await db.logAccess(user.id, {
      device: deviceName,
      ip: clientIp
    });
...
function getBaseUrl(req) {
  // Si hay un header X-Forwarded-Proto, usarlo (para proxies/load balancers)
