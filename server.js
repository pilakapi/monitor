...
/**
 * DELETE /api/playlists/:id/devices
 * Elimina todos los dispositivos conectados de una playlist
 */
app.delete('/api/playlists/:id/devices', requireAuth, async (req, res) => {
  try {
    const { id } = req.params;

    await db.pool.query('DELETE FROM connected_devices WHERE playlist_id = $1', [id]);

    res.json({ success: true, message: 'Todos los dispositivos desconectados' });
  } catch (error) {
    console.error('Error al desconectar dispositivos:', error);
    res.status(500).json({ success: false, message: 'Error al desconectar dispositivos' });
  }
});

/**
 * DELETE /api/playlists/:id/devices/:deviceIp
 * Elimina un dispositivo específico de una playlist
 */
app.delete('/api/playlists/:id/devices/:deviceIp', requireAuth, async (req, res) => {
  try {
    const { id, deviceIp } = req.params;

    await db.pool.query(
      'DELETE FROM connected_devices WHERE playlist_id = $1 AND ip_address = $2',
      [id, deviceIp]
    );

    res.json({ success: true, message: 'Dispositivo desconectado' });
  } catch (error) {
    console.error('Error al desconectar dispositivo:', error);
    res.status(500).json({ success: false, message: 'Error al desconectar dispositivo' });
  }
});

/**
 * GET /api/playlists/:id/devices
 * Obtiene la lista de dispositivos conectados de una playlist
 */
app.get('/api/playlists/:id/devices', requireAuth, async (req, res) => {
  try {
    const { id } = req.params;

    // Limpiar dispositivos expirados primero
    await db.cleanupExpiredDevices();

    // Obtener todos los dispositivos (activos e inactivos)
    const query = `
      SELECT
        ip_address,
        user_agent,
...
function getClientIp(req) {
  ...
  // Limpiar "::ffff:" si es IPv4 mapeada a IPv6
...
/**
 * Función helper para generar un identificador único de dispositivo
 * Combina IP + User-Agent para identificar dispositivos únicos
 * Esto es útil cuando múltiples dispositivos comparten la misma IP (red local)
 */
function getDeviceIdentifier(req) {
  const clientIp = getClientIp(req);
  const userAgent = req.headers['user-agent'] || 'unknown';

  // Si es IP local/loopback, usar User-Agent como identificador adicional
  ...
  // Crear un hash único basado en IP + User-Agent
  const identifier = `${clientIp}|${userAgent.substring(0, 100)}`;
  const deviceHash = crypto.createHash('md5').update(identifier).digest('hex');

  return {
    ip: clientIp,
    deviceHash: isLocal ? deviceHash : clientIp, // Usar hash solo para IPs locales
    isLocal
  };
}
...
app.get('/get/:mirror_id.m3u', async (req, res) => {
  try {
    ...
    const deviceId = deviceInfo.deviceHash;
    const userAgent = req.headers['user-agent'] || 'unknown';

    console.log(`[M3U Proxy] Solicitud para ${mirror_id} desde IP: ${clientIp} (Local: ${deviceInfo.isLocal}, ID: ${deviceId.substring(0, 8)}...)`);
    ...
    // Limpiar dispositivos expirados antes de verificar
    await db.cleanupExpiredDevices();

    // Obtener el conteo actual de dispositivos activos directamente
    const countResult = await db.pool.query(
      'SELECT COUNT(*) as count FROM connected_devices WHERE playlist_id = $1 AND is_active = true',
      [playlist.id]
    );
    const currentActiveCount = parseInt(countResult.rows[0].count);

    console.log(`[M3U Proxy] Dispositivos activos actuales: ${currentActiveCount}/${maxDevices}`);

    // Verificar si este dispositivo específico ya está conectado (usando deviceId)
    const currentDeviceResult = await db.pool.query(
      'SELECT is_active FROM connected_devices WHERE playlist_id = $1 AND ip_address = $2',
      [playlist.id, deviceId]
    );

    const isDeviceAlreadyConnected = currentDeviceResult.rows.length > 0 && currentDeviceResult.rows[0].is_active;

    // Si el dispositivo ya está conectado, solo actualizamos su heartbeat
    if (isDeviceAlreadyConnected) {
      await db.pool.query(
        'UPDATE connected_devices SET last_heartbeat_at = CURRENT_TIMESTAMP, user_agent = $3 WHERE playlist_id = $1 AND ip_address = $2',
        [playlist.id, deviceId, userAgent]
      );
      console.log(`[M3U Proxy] Dispositivo existente actualizado: ${deviceId.substring(0, 8)}...`);
    } else {
      // Es un dispositivo nuevo - verificar el límite primero
      if (currentActiveCount >= maxDevices) {
        console.log(`[M3U Proxy] Límite alcanzado para ${mirror_id}: ${currentActiveCount}/${maxDevices}`);
      ...
      const deviceResult = await db.registerDevice(playlist.id, deviceId, userAgent);

      if (!deviceResult.success) {
        console.log(`[M3U Proxy] Error al registrar dispositivo: ${deviceResult.message}`);
        return res.status(500).type('application/x-mpegurl')
          .send('#EXTM3U\n#EXTINF:-1,ERROR INTERNO\n#EXTINF:-1,Error: Error al registrar dispositivo\n');
      }

      console.log(`[M3U Proxy] Nuevo dispositivo registrado: ${deviceId.substring(0, 8)}... (Total: ${currentActiveCount + 1})`);
    }

    // Registrar el acceso en el historial (guardamos la IP real también para referencia)
