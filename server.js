...
app.get('/get/:mirror_id.m3u', async (req, res) => {
  try {
    ...
    // Obtener el estado actual de dispositivos (para verificar si este IP ya está registrado)
    const currentDevicesResult = await db.pool.query(
      'SELECT is_active FROM connected_devices WHERE playlist_id = $1 AND ip_address = $2',
      [playlist.id, clientIp]
    );

    const isExistingDevice = currentDevicesResult.rows.length > 0;
    const isDeviceActive = isExistingDevice && currentDevicesResult.rows[0].is_active;

    // Si el dispositivo ya está activo, simplemente actualizamos su heartbeat
    if (isDeviceActive) {
      await db.pool.query(
        'UPDATE connected_devices SET last_heartbeat_at = CURRENT_TIMESTAMP WHERE playlist_id = $1 AND ip_address = $2',
        [playlist.id, clientIp]
      );
      ...
      if (activeDevices >= maxDevices) {
        console.log(`[M3U Proxy] Límite alcanzado para ${mirror_id}: ${activeDevices}/${maxDevices}`);
      ...
      const deviceResult = await db.registerDevice(playlist.id, clientIp, userAgent);
      ...
      console.log(`[M3U Proxy] Nuevo dispositivo registrado: ${clientIp}`);
    }
