/**
 * M3U Monitor Proxy - Módulo de Base de Datos
 * Conexión a Neon PostgreSQL para persistencia de datos
 */

const { Pool } = require('pg');

// Crear pool de conexión usando la variable de entorno DATABASE_URL
// En Render, esta variable se configura en el dashboard
const pool = new Pool({
  connectionString: process.env.DATABASE_URL,
  ssl: {
    rejectUnauthorized: false // Necesario para Neon en modo gratuito
  }
});

/**
 * Inicializar la base de datos creando las tablas necesarias
 */
async function initializeDatabase() {
  const createTableQuery = `
    CREATE TABLE IF NOT EXISTS users (
      id UUID PRIMARY KEY,
      slug VARCHAR(100) UNIQUE NOT NULL,
      name VARCHAR(255) NOT NULL,
      cedula VARCHAR(50) NOT NULL,
      phone VARCHAR(50) NOT NULL,
      original_url TEXT NOT NULL,
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      last_access TIMESTAMP,
      last_device VARCHAR(50),
      total_connections INTEGER DEFAULT 0,
      access_logs JSONB DEFAULT '[]'
    );

    CREATE INDEX IF NOT EXISTS idx_users_name ON users(name);
    CREATE INDEX IF NOT EXISTS idx_users_cedula ON users(cedula);
    CREATE INDEX IF NOT EXISTS idx_users_slug ON users(slug);
  `;

  try {
    await pool.query(createTableQuery);
    console.log('✅ Base de datos inicializada correctamente');
  } catch (error) {
    console.error('❌ Error al inicializar la base de datos:', error.message);
    throw error;
  }
}

/**
 * Generar slug URL-friendly desde el nombre
 */
function generateSlug(name) {
  // Convertir a minúsculas, reemplazar espacios con guiones, remover caracteres especiales
  return name
    .toLowerCase()
    .trim()
    .replace(/[áéíóúñ]/g, match => 'aeiou'n'.replace(match, ''))
    .replace(/[^a-z0-9\s-]/g, '')
    .replace(/\s+/g, '-')
    .replace(/-+/g, '-')
    .substring(0, 50);
}

/**
 * Obtener todos los usuarios (con filtro opcional por nombre y conteo acumulativo)
 */
async function getUsers(searchName = null) {
  try {
    let query = 'SELECT * FROM users';
    let params = [];

    if (searchName) {
      query += ' WHERE name ILIKE $1';
      params = [`%${searchName}%`];
    }

    query += ' ORDER BY created_at DESC';

    const result = await pool.query(query, params);
    return result.rows;
  } catch (error) {
    console.error('❌ Error al obtener usuarios:', error.message);
    throw error;
  }
}

/**
 * Obtener un usuario por su ID
 */
async function getUserById(id) {
  try {
    const result = await pool.query('SELECT * FROM users WHERE id = $1', [id]);
    return result.rows[0];
  } catch (error) {
    console.error('❌ Error al obtener usuario:', error.message);
    throw error;
  }
}

/**
 * Obtener usuario por slug (URL personalizada)
 */
async function getUserBySlug(slug) {
  try {
    const result = await pool.query('SELECT * FROM users WHERE slug = $1', [slug]);
    return result.rows[0];
  } catch (error) {
    console.error('❌ Error al obtener usuario por slug:', error.message);
    throw error;
  }
}

/**
 * Verificar si un slug ya existe
 */
async function slugExists(slug, excludeId = null) {
  try {
    let query = 'SELECT COUNT(*) as count FROM users WHERE slug = $1';
    const params = [slug];

    if (excludeId) {
      query += ' AND id != $2';
      params.push(excludeId);
    }

    const result = await pool.query(query, params);
    return parseInt(result.rows[0].count) > 0;
  } catch (error) {
    console.error('❌ Error al verificar slug:', error.message);
    throw error;
  }
}

/**
 * Crear un nuevo usuario con slug único
 */
async function createUser(user) {
  const { id, name, cedula, phone, original_url } = user;

  try {
    // Generar slug único
    let baseSlug = generateSlug(name);
    let slug = baseSlug;
    let counter = 1;

    // Asegurar que el slug sea único
    while (await slugExists(slug)) {
      slug = `${baseSlug}-${counter}`;
      counter++;
    }

    const result = await pool.query(
      `INSERT INTO users (id, slug, name, cedula, phone, original_url, created_at, total_connections)
       VALUES ($1, $2, $3, $4, $5, $6, CURRENT_TIMESTAMP, 0)
       RETURNING *`,
      [id, slug, name, cedula, phone, original_url]
    );
    return result.rows[0];
  } catch (error) {
    console.error('❌ Error al crear usuario:', error.message);
    throw error;
  }
}

/**
 * Actualizar un usuario existente
 * El ID no se puede cambiar - mantiene la URL espejo constante
 * El slug solo cambia si el nombre cambia y no hay conflictos
 */
async function updateUser(id, updates) {
  const { name, cedula, phone, original_url } = updates;

  try {
    // Obtener usuario actual
    const currentUser = await getUserById(id);
    if (!currentUser) {
      throw new Error('Usuario no encontrado');
    }

    let slug = currentUser.slug;

    // Si el nombre cambió, actualizar el slug
    if (name !== currentUser.name) {
      let baseSlug = generateSlug(name);
      let newSlug = baseSlug;
      let counter = 1;

      // Asegurar que el slug sea único
      while (await slugExists(newSlug, id)) {
        newSlug = `${baseSlug}-${counter}`;
        counter++;
      }

      slug = newSlug;
    }

    const result = await pool.query(
      `UPDATE users
       SET name = $1, cedula = $2, phone = $3, original_url = $4, slug = $5
       WHERE id = $6
       RETURNING *`,
      [name, cedula, phone, original_url, slug, id]
    );
    return result.rows[0];
  } catch (error) {
    console.error('❌ Error al actualizar usuario:', error.message);
    throw error;
  }
}

/**
 * Eliminar un usuario
 */
async function deleteUser(id) {
  try {
    await pool.query('DELETE FROM users WHERE id = $1', [id]);
    return true;
  } catch (error) {
    console.error('❌ Error al eliminar usuario:', error.message);
    throw error;
  }
}

/**
 * Registrar acceso del dispositivo y actualizar información
 * Incrementa el contador total de conexiones de forma acumulativa
 */
async function logAccess(id, deviceInfo) {
  const { device, ip } = deviceInfo;

  try {
    // Obtener el usuario actual para agregar al log
    const user = await getUserById(id);
    if (!user) return null;

    // Crear nuevo registro de acceso
    const newLog = {
      date: new Date().toISOString(),
      device: device,
      ip: ip
    };

    // Agregar al historial de accesos
    const accessLogs = user.access_logs || [];
    accessLogs.push(newLog);

    // Mantener solo los últimos 100 accesos
    const trimmedLogs = accessLogs.slice(-100);

    // Incrementar contador total de conexiones
    const newTotalConnections = (user.total_connections || 0) + 1;

    // Actualizar usuario con último acceso y contador
    const result = await pool.query(
      `UPDATE users
       SET last_access = CURRENT_TIMESTAMP,
           last_device = $1,
           access_logs = $2,
           total_connections = $3
       WHERE id = $4
       RETURNING *`,
      [device, JSON.stringify(trimmedLogs), newTotalConnections, id]
    );

    return result.rows[0];
  } catch (error) {
    console.error('❌ Error al registrar acceso:', error.message);
    throw error;
  }
}

/**
 * Cerrar el pool de conexiones
 */
async function closePool() {
  await pool.end();
  console.log('🔌 Conexiones de base de datos cerradas');
}

module.exports = {
  pool,
  initializeDatabase,
  generateSlug,
  getUsers,
  getUserById,
  getUserBySlug,
  slugExists,
  createUser,
  updateUser,
  deleteUser,
  logAccess,
  closePool
};
